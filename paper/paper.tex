...
\section{Results}
	...
	The most common TCP congestion algorithms are CTCP, CUBIC and BIC. CTCP is the default congestion algorithm in Microsoft Windows Server Editions and Microsoft Windows 8 and newer [SRC]. CUBIC is the default congestion algorithm in Linux distributions with kernel versions ... and newer [SRC]. BIC was the default congestion algorithm in earlier Linux distributions with kernel versions: [SRC]. DCTCP and BBR will also be tested as those protocols are newer. DCTCP is specifically designed for datacenters [SRC] and BBR is a new congestion protocol not yet widely deployed [SRC]. 
