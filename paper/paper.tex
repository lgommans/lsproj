...
fair towards all other algorithms.

CTCP is the only algorithm that responds excessively to others. Alone, it
reaches the full gigabit speed; but once any other algorithm runs at the same
...
On top of this, CTCP also lowers its speed in response to the simulated network
...
speeds, but we see no signs of BBR being the cause of that. In tests that
involve two algorithms that are neither BBR nor CTCP, speeds are not higher
than with BBR being one of the two algorithms. %TODO check this one (this was gut feeling, but looking at the data, there might be a slight effect that BBR has on things)
